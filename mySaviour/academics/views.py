...
def course(request, year, branch_id):
    ...
    print("Year: ", year)
    year = Year.objects.filter(number = int(year))[0]
    print(year.number)
    # print(courses)
...
def documents(request, course_id, year):
    print(year)
    year_ob = Year.objects.filter(number = int(year))
    print("DOCUMENTS", year_ob) 
...
def show_documents(request, course_id, year_number, type_id):
    print("Show", year_number)
    document_type = Document_type.objects.filter(id = type_id)
    year = Year.objects.filter(number = year_number)
